...
## CURRENT V1 STATUS — 18 September 2026

The software MVP described below is now implemented across `app-patient`, `app-family`, and `core-model`.

Release gates pass for core tests, both APK builds, and both lint tasks. Emulator regression QA on `emulator-5554` passes for Patient -> Family events, repeated-pain alert/notification, restart persistence, Arabic/RTL, low vision, protected clear, and canvas lock.

Per user instruction, OnePlus 7 is excluded from current/future QA. Debug builds include an emulator-only local event receiver; production behavior retains BLE.

Current implementation report: `docs/V1_IMPLEMENTATION.md`.
